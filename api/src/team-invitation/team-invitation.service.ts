...
export class TeamInvitationService {
    ...
    public async createTeamInvitation(body: AddTeamInvitationDTO, user: User): Promise<ShowTeamInvitationDTO> {
        ...
        const createdInvitation = await this.teamInvitationRepository.save(newInvitation);
        const inviteeToDTO = plainToClass(ShowUserDTO, await invitee, { excludeExtraneousValues: true });
        const teamToDTO = plainToClass(ShowTeamDTO, await team, { excludeExtraneousValues: true });
        const statusToDTO = plainToClass(ShowTeamInvitationStatusDTO, await status, { excludeExtraneousValues: true });
        const exposedInvitation: ShowTeamInvitationDTO = plainToClass(ShowTeamInvitationDTO, createdInvitation, { excludeExtraneousValues: true });
    ...
    public async acceptInvitation(invitationId): Promise<ShowTeamInvitationDTO> {
      ...
      const acceptedInvitation: TeamInvitation = await this.teamInvitationRepository.save(invitation);
      const statusToDTO = plainToClass(ShowTeamInvitationStatusDTO, await newStatus, { excludeExtraneousValues: true });
      const acceptInvitationToDTO = plainToClass(ShowTeamInvitationDTO, await acceptedInvitation, { excludeExtraneousValues: true });
    ...
    public async rejectInvitation(invitationId): Promise<ShowTeamInvitationDTO> {
      ...
      const statusToDTO = plainToClass(ShowTeamInvitationStatusDTO, await newStatus, { excludeExtraneousValues: true });
      const rejectInvitationToDTO = plainToClass(ShowTeamInvitationDTO, await rejectedInvitation, { excludeExtraneousValues: true });
      rejectInvitationToDTO.status = statusToDTO;
      return await rejectInvitationToDTO;
    }

    public async getActiveInvitationsByUserId(userId): Promise<ShowTeamInvitationDTO[]> {
      ...
      activeInvitations.forEach(inv => {
        const invToDTO  = plainToClass(ShowTeamInvitationDTO, inv, { excludeExtraneousValues: true });
